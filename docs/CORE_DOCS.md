# Core Documentation Index (2.9.7)

Documents in this index define or support the **stable core** release line.

## Specification

- [`../SPEC.md`](../SPEC.md)
- [`LANGUAGE_SPEC_2_9.md`](LANGUAGE_SPEC_2_9.md)
- [`LANGUAGE_SPEC_3_0.md`](LANGUAGE_SPEC_3_0.md) (draft v3 track)
- [`../spec/mellow-2.9-core.json`](../spec/mellow-2.9-core.json)
- [`../spec/mellow-2.9.7-stability.json`](../spec/mellow-2.9.7-stability.json)
- [`../spec/mellow-2.9.4-stability.json`](../spec/mellow-2.9.4-stability.json)

## Language and runtime

- [`SYNTAX_REFERENCE.md`](SYNTAX_REFERENCE.md)
- [`STABLE_CORE.md`](STABLE_CORE.md)
- [`STABLE_CHECKLIST.md`](STABLE_CHECKLIST.md)
- [`RUNTIME_PARITY_3_0.md`](RUNTIME_PARITY_3_0.md)
- [`V3_STABILITY_PLAN.md`](V3_STABILITY_PLAN.md)
- [`COMPILER_V3.md`](COMPILER_V3.md)

## Packages and SDK

- [`CORE_PACKAGES.md`](CORE_PACKAGES.md)
- [`PACKAGE_SYSTEM_2_9.md`](PACKAGE_SYSTEM_2_9.md)
- [`MELLOW_SDK.md`](MELLOW_SDK.md)
- [`LOCKFILE_AND_IMPORTS.md`](LOCKFILE_AND_IMPORTS.md)
- [`INTEROP.md`](INTEROP.md)

## Tooling

- [`CLI_UX.md`](CLI_UX.md)
- [`TESTING.md`](TESTING.md)
- [`LSP.md`](LSP.md)

## Domain cores (stable host modules)

- [`DATA_PROCESSING_CORE.md`](DATA_PROCESSING_CORE.md)
- [`LEDGER_CORE.md`](LEDGER_CORE.md)
- [`STDLIB_REFERENCE.md`](STDLIB_REFERENCE.md)

## Not core

Experimental and extended surfaces are indexed in
[`experimental/README.md`](experimental/README.md).
