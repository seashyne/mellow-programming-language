...
DEFAULT_REGISTRY = os.environ.get("MELLOW_REGISTRY_URL", "https://registry.mellowlang.org")
CLIENT_USER_AGENT = f"MellowCLI/{os.environ.get('MELLOW_CLI_VERSION', '1.6.4')} (+https://mellowlang.org)"
...
def install_local_package_into_project(name: str, project_dir: str | Path, version: str | None = None, *, with_deps: bool = True, _visited: set[str] | None = None) -> Dict[str, Any]:
    ...
    if visit_key in visited:
        return {"ok": True, "name": base_name, "version": resolved_version, "installed_to": str(dst.parent), "installed": installed, "lockfile": str(lockfile_path), "aliases_file": str(alias_path), "source_kind": source_kind}
...
keep app = win.window(title: "Mellow Desktop App", width: 960, height: 640, layout: "vstack")
win.menu(app, "File", ["About"])
win.menu_item(app, "File", "Quit", "close")
win.label(app, "Hello {{state.name}}")
win.input(app, "Mellow", bind: name)
...
keep payload = {"kind": "nightly.sync", "at": time.now_iso()}
...
def set_registry(url: str) -> Dict[str, Any]:
    ...
    save_config(cfg)
    return {"registry": cfg["registry"], "config": str(CONFIG_FILE)}
...
def clear_auth_token(registry: str | None = None) -> Dict[str, Any]:
    ...
    auth.pop(reg, None)
    save_config(cfg)
    return {"ok": True, "registry": reg, "saved_to": str(CONFIG_FILE)}
...
def init_package(target_dir: str | Path, name: str | None = None, entry: str = "main.mellow") -> Dict[str, Any]:
    ensure_dirs()
    td = Path(target_dir)
    ...
    if not mf.exists():
        mf.write_text(f'function hello() {{\n    print("Hello from {pkg_name}")\n}}\n', encoding="utf-8")
...
def _pkg_cache_path(name: str, version: str) -> Path:
    ensure_dirs()
    return CACHE_ROOT / normalize_name(name).replace("/", "__").replace("@", "at_") / f"{version}.mpkg"
...
def sign_manifest(manifest: Dict[str, Any], sha256: str) -> Dict[str, Any]:
    ...
    priv = serialization.load_pem_private_key((KEYS_DIR / "ed25519_private.pem").read_bytes(), password=None)
    pub_bytes = (KEYS_DIR / "ed25519_public.pem").read_bytes()
...
def update_remote(name: str | None = None, registry: str | None = None, *, project_dir: str | Path | None = None, with_deps: bool = True) -> Dict[str, Any]:
    ...
    for pkg_name, spec in targets:
        if pkg_name in seen:
            continue
        seen.add(pkg_name)
        res = install_remote(pkg_name, version=spec, registry=reg, with_deps=with_deps, project_dir=project_dir)
        if not res.get("ok"):
            return res
        updated.extend(res.get("installed", [{"name": pkg_name, "version": res.get("version")}] ))
    return {"ok": True, "registry": reg, "updated": updated, "count": len(updated), "lockfile": str(_lockfile_path(project_dir))}
...
def publish_from_dir(package_dir: str | Path) -> Dict[str, Any]:
    ...
    return {"name": name, "version": version, "published_to": str(out_dir)}
...
def install_package(name: str, version: str | None = None) -> Dict[str, Any]:
    ...
    manifest = read_manifest(src)
    (dst / "manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return {"name": name, "version": ver, "installed_to": str(dst.parent)}
...
def login_with_token(token: str, registry: str | None = None) -> Dict[str, Any]:
    ...
    if not probe.get("ok"):
        ...
        return probe
    set_auth_token(reg, token)
    probe["saved_to"] = str(CONFIG_FILE)
...
def search_remote(query: str, registry: str | None = None) -> Dict[str, Any]:
    reg = get_registry_url(registry)
    token = get_auth_token(reg)
    return _request_json("GET", reg + "/api/v1/packages/search?" + urllib.parse.urlencode({"q": query}), token=token)
...
def publish_remote(package_dir: str | Path, registry: str | None = None, token: str | None = None) -> Dict[str, Any]:
    ...
    return _request_json("POST", reg + "/api/v1/packages/publish", payload=payload, token=auth)
...
def _extract_archive_to_install(name: str, version: str, raw: bytes) -> Dict[str, Any]:
    ...
    return {"ok": True, "name": name, "version": version, "installed_to": str(target.parent)}
