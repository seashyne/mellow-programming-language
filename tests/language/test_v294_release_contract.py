...
MANIFEST = ROOT / "spec" / "mellow-2.9.7-stability.json"
CORE_DOCS = ROOT / "docs" / "CORE_DOCS.md"


def test_release_version_is_2_9_6() -> None:
    assert mellowlang.__version__ == "2.9.7"


def test_patch_stability_manifest_exists() -> None:
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert manifest["version"] == "2.9.7"
    assert manifest["status"] in {"in-development", "released"}
...
def test_core_docs_index_points_to_current_patch() -> None:
    ...
    assert "mellow-2.9.7-stability.json" in text
